...
const viewLanguage = (item) => {
  ...
  <div class="auth-container">
    <h4>${item.language} ${item.definition}</h4>
    <h5>Author Email: ${item.firebaseKey}</h5>
  </div>
  <div class="card-contain">
  `;

  item.vocabObject.forEach((arr) => {
    domString += `<div class="card">
    <img class="card-img-top" src=${arr.image} alt=${arr.title} style="height: 400px;">
    <div class="card-body" style="height: 180px;">
      <h5 class="card-title">${arr.title}</h5>
        <p class="card-text bold">${arr.favorite ? `<span class="badge badge-info sale-badge"><i class="fa fa-bell" aria-hidden="true"></i> Sale</span> $${arr.price}` : `$${arr.price}`}</p>
        <hr>
